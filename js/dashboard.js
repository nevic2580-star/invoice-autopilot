...
const Dashboard = (() => {
  ...
  let selectedVesselName = 'ALL';

  function renderDashboard(lang='ko') {
    const isKo=lang==='ko', t=(k,e)=>isKo?k:e;
    const allInvoices=Storage.getInvoices();
    let invoices = selectedVesselType === 'ALL' ? allInvoices : filterByVesselType(allInvoices, selectedVesselType);
    if(selectedVesselName !== 'ALL') {
      invoices = invoices.filter(i => i.vesselName === selectedVesselName);
    }
    ...
    <div class="page-body">
      ...
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
        <div class="vessel-type-tabs" style="margin-bottom:0;">
          <button class="vessel-type-tab ${selectedVesselType==='ALL'?'active':''}" onclick="Dashboard.setVesselType('ALL')">
            🚢 ${t('전체','All')} (${selectedVesselType==='ALL'?'ALL':filterByVesselType(allInvoices, selectedVesselType).length})
          </button>
          ${vesselTypes.map(vt => `
        ...
        <select class="form-select" onchange="Dashboard.setVesselName(this.value)" style="width:250px; background-color:var(--bg-level2);">
          <option value="ALL">-- ${t('특정 선박 선택 (Specific Vessel)','Specific Vessel')} --</option>
          ${Storage.getVessels().filter(v=>selectedVesselType==='ALL'||v.vesselType===selectedVesselType).map(v=>`<option value="${v.vesselName}" ${selectedVesselName===v.vesselName?'selected':''}>${v.vesselName}</option>`).join('')}
        </select>
      </div>
      ...
      <div class="charts-grid">
        ...
        <div class="chart-container animate-in">
          <div class="card-header"><h3>🔧 ${t('Kind별 현황','Kind Cost Overview')}</h3></div>
      ...
      <div class="charts-grid" style="grid-template-columns:1fr 1fr">
        ...
        <div class="chart-container animate-in">
          <div class="card-header"><h3>🏆 ${t('상위 청구 업체','Top Vendors by Cost')}</h3></div>
          <div style="padding-top:10px;">
            ${renderTopVendors(invoices,isKo)}
          </div>
        </div>
      </div>
  ...
  function renderTopVendors(invoices, isKo) {
    if(!invoices.length)return `<div style="text-align:center;padding:30px;color:var(--text-muted)">✅ ${isKo?'데이터 없음':'No data'}</div>`;
    
    const vendorMap = {};
    invoices.forEach(inv => {
      if(inv.status === '입력') return; // Exclude drafts from spend analysis
      const v = inv.vendorName || 'Unknown';
      if(!vendorMap[v]) vendorMap[v] = 0;
      vendorMap[v] += AnalysisEngine.toUSD(inv.actualAmount || inv.estimatedAmount || 0, inv.currency);
    });
    ...
    if(!topVendors.length) return `<div style="text-align:center;padding:30px;color:var(--text-muted)">✅ ${isKo?'청구 금액 없음':'No billing data'}</div>`;
    
    const maxVal = topVendors[0][1] || 1;
    
    return topVendors.map(([vendor, val]) => `
      <div style="margin-bottom:14px;">
        <div style="display:flex;justify-content:space-between;margin-bottom:5px;font-size:0.85rem;">
          <span style="font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:60%;" title="${vendor}">${vendor}</span>
          <span style="color:var(--primary-color);font-weight:700;">$${Math.round(val).toLocaleString()}</span>
        </div>
        <div style="width:100%;background:rgba(255,255,255,0.05);border-radius:4px;height:6px;overflow:hidden;">
          <div class="animate-width" style="width:${(val/maxVal)*100}%;background:linear-gradient(90deg, var(--primary-color), var(--accent-color));height:100%;border-radius:4px;transition:width 1s ease-out;"></div>
        </div>
      </div>
    `).join('');
  }
  ...
  function drawCharts() {
    const allInvoices=Storage.getInvoices();
    let invoices = selectedVesselType === 'ALL' ? allInvoices : filterByVesselType(allInvoices, selectedVesselType);
    if(selectedVesselName !== 'ALL') invoices = invoices.filter(i=>i.vesselName===selectedVesselName);
    drawGaugeChart(invoices);
    drawTrendChart(invoices);
  ...
  function drawTrendChart(invoices) {
    ...
    // Legend
    ctx.setLineDash([4, 4]); ctx.beginPath(); ctx.moveTo(w - 170, 8); ctx.lineTo(w - 150, 8);
    ctx.strokeStyle = '#64748b'; ctx.lineWidth = 2; ctx.stroke(); ctx.setLineDash([]);
    ctx.fillStyle = '#94a3b8'; ctx.font = '10px Inter'; ctx.textAlign = 'left'; ctx.fillText('견적(Est.)', w - 146, 11);

    ctx.beginPath(); ctx.moveTo(w - 80, 8); ctx.lineTo(w - 60, 8);
    ctx.strokeStyle = '#3b82f6'; ctx.lineWidth = 3; ctx.stroke();
    ctx.fillStyle = '#94a3b8'; ctx.fillText('실적(Actual)', w - 56, 11);
  }
  ...
  function drawVesselChart(invoices) {
    ...
    ctx.fillStyle='#3b82f6';ctx.fillRect(w-160,8,10,10);ctx.fillStyle='#94a3b8';ctx.font='10px Inter';ctx.textAlign='left';ctx.fillText('견적(Est.)',w-146,17);
    ctx.fillStyle='#f59e0b';ctx.fillRect(w-80,8,10,10);ctx.fillStyle='#94a3b8';ctx.fillText('실적(Actual)',w-66,17);
  }
  ...
  function drawKindChart(invoices) {
    ...
    invoices.forEach(i=>{
      ...
      if(!groups[kName]) groups[kName] = {est:0, act:0};
      groups[kName].est += AnalysisEngine.toUSD(i.estimatedAmount,i.currency);
      groups[kName].act += AnalysisEngine.toUSD(i.actualAmount,i.currency);
    });
    
    const entries=Object.entries(groups).sort((a,b)=>b[1].act-a[1].act).slice(0, 6);
    if(!entries.length) return;
    
    const maxVal=Math.max(...entries.map(e=>Math.max(e[1].est,e[1].act)), 1);
    const barW=(w-80)/(entries.length*3);
    const chartH=h-60;
    ...
    entries.forEach(([name,data],idx)=>{
      ...
      const estH=(data.est/maxVal)*chartH;const actH=(data.act/maxVal)*chartH;
      const gEst=ctx.createLinearGradient(0,30+chartH-estH,0,30+chartH);
      gEst.addColorStop(0,'#8b5cf6');gEst.addColorStop(1,'#6d28d9'); // Purple for kind Est
      ctx.fillStyle=gEst;ctx.beginPath();roundRect(ctx,x,30+chartH-estH,barW,estH,4);ctx.fill();
      const gAct=ctx.createLinearGradient(0,30+chartH-actH,0,30+chartH);
      gAct.addColorStop(0,data.act>data.est?'#ef4444':'#10b981');gAct.addColorStop(1,data.act>data.est?'#b91c1c':'#059669');
      ctx.fillStyle=gAct;ctx.beginPath();roundRect(ctx,x+barW+4,30+chartH-actH,barW,actH,4);ctx.fill();
      ctx.fillStyle='#94a3b8';ctx.font='9px Noto Sans KR';ctx.textAlign='center';
      const label=name.length>10?name.slice(0,10)+'…':name;
      ctx.fillText(label,x+barW,30+chartH+14);
    });
    ctx.fillStyle='#8b5cf6';ctx.fillRect(w-160,8,10,10);ctx.fillStyle='#94a3b8';ctx.font='10px Inter';ctx.textAlign='left';ctx.fillText('견적(Est.)',w-146,17);
    ctx.fillStyle='#10b981';ctx.fillRect(w-80,8,10,10);ctx.fillStyle='#94a3b8';ctx.fillText('실적(Actual)',w-66,17);
  }
  ...
  return { renderDashboard, drawCharts, setVesselType, setVesselName };
})();
