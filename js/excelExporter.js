...
const ExcelExporter = (() => {
  ...
  function exportInvoiceList() {
    ...
    const headers = [
      ...
      '분류 (Category)', '수리코드 (Repair Code)', '수리내역 (Repair Desc)', '작업내역 (Work Description)', '통화 (Currency)',
    ...
    const rows = invoices.map(i => [
      ...
      i.category, i.repairCode || '-', i.repairDesc || '-', i.workDescription, i.currency,
    ...
    ws['!cols'] = [
      ...
      { wch: 14 }, { wch: 15 }, { wch: 25 }, { wch: 30 }, { wch: 8 },
    ...
    const activeUser = (typeof App !== 'undefined' && App.getCurrentUser) ? App.getCurrentUser() : null;
    if (activeUser && activeUser.role === 'vendor') {
      const fileName = `인보이스_진행상황_${new Date().toISOString().slice(0, 10)}.xlsx`;
      XLSX.writeFile(wb, fileName);
      App.showToast('success', '다운로드 완료', `${fileName} 파일이 저장되었습니다.`);
      if (typeof NotificationCenter !== 'undefined') {
        NotificationCenter.logActivity({ action: 'export', detail: '협력사 Excel 인보이스 목록 다운로드' });
      }
      return;
    }
  ...
  function exportAdvancePayment(vesselGroups, accountGroups, categoryGroups, kindGroups, grandTotal, month, invoices) {
    ...
    // Sheet 1: List Detail
    const headers1 = ['선박명 (Vessel)', '업체명 (Vendor)', '계정 (ACCT. NO)', '비용구분 (Cost Cat.)', '기기분류 (Eq Cat.)', '수리코드 (Repair Code)', '수리내용 (Repair Desc)', '원본 통화 (Currency)', '원본 금액 (Orig. Amt)', 'USD 금액 (USD Amt)'];
    const rows1 = invoices.map(i => [
      i.vesselName, i.vendorName || '-', i.acctNo || '-', i.category || '기타', i.kind || '-', i.repairCode || '-', i.repairDesc || '-', i.currency, i.actualAmount || i.estimatedAmount, i.usdAmount
    ]);
    const ws1 = XLSX.utils.aoa_to_sheet([[`선급금 대상 목록 - ${month}`], [], headers1, ...rows1]);
    ws1['!cols'] = [{ wch: 22 }, { wch: 20 }, { wch: 15 }, { wch: 15 }, { wch: 15 }, { wch: 18 }, { wch: 25 }, { wch: 15 }, { wch: 15 }, { wch: 15 }];
