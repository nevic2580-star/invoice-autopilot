  ...
  function processApproval(invoiceId, type, result, comment = '') {
    ...
    if (type === 'supervisor') {
      ...
      if (result === 'approved') {
        ...
        // Reject: return to supervisor stage (not back to 접수)
        invoice.status = '감독확인완료';
        // Clear only team lead record, keep supervisor approval intact
        invoice.approval.teamLead = { name: null, date: null, status: 'pending', comment: '' };
      }
    }
