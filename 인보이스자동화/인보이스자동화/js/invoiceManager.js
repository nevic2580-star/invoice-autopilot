...
const InvoiceManager = (() => {
  ...
  function renderRegisterPage(lang='ko', editId=null) {
    ...
    let initialCountry = inv?.workCountry || '';
    if (!initialCountry && inv?.workPort) {
      const matchPort = ports.find(p => p.name === inv.workPort);
      if (matchPort) initialCountry = matchPort.country;
    }
    const countryOptsHtml = countries.map(c =>
      `<option value="${c.code}" ${initialCountry === c.code ? 'selected' : ''}>${c.name}</option>`
    ).join('');
    const portOptsHtml = initialCountry
      ? ports.filter(p => p.country === initialCountry).map(p =>
          `<option value="${p.name}" ${inv?.workPort === p.name ? 'selected' : ''}>${p.nameKo} (${p.name})</option>`
        ).join('')
      : '';
            ...
            <div class="glass-card animate-in" style="margin-bottom:16px">
              ...
              <div class="form-grid-3">
                ...
                <div class="form-group"><label class="form-label">실제발생금액 (Actual) <span class="required">*</span></label>
                  <input class="form-input" type="text" id="actualAmount" required value="${inv?.actualAmount != null && inv.actualAmount !== '' ? Number(inv.actualAmount).toLocaleString() : ''}" oninput="InvoiceManager.formatNumberInput(this)" onblur="InvoiceManager.calcDifference()"></div>
              </div>
              <div class="form-grid-2" ${hideAdmin}>
                ...
                <div class="form-group"><label class="form-label">작업항구 (Port) <span class="required">*</span></label>
                  <div style="display:flex;gap:8px;">
                    <select class="form-select" id="workPort" required style="flex:1;">
                      <option value="">${initialCountry ? '-- 선택 (Select) --' : '국가를 먼저 선택하세요'}</option>
                      ${portOptsHtml}
                    </select>
